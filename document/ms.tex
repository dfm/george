...
In this \paper, we demonstrate a practical technique for applying a GP model
to real \kepler\ data by taking advantage of the sparsity of the problem and
recent advances in computational linear algebra (Ambikasaran \etal, in prep).
...
The most commonly used likelihood function assumes that the error bars on the
data are known, independent, and Gaussian.
In this case, the log-likelihood of the data is
...
\end{eqnarray}
In \eq{chi2}, $m(t_n;\,\bvec{\theta})$ is your generative model for the
...
Instead, it is common practice to choose a parametric \emph{kernel function}
that defines the elements of the covariance matrix as a function of the
relative independent coordinates (times) between the data points
\begin{eqnarray}
