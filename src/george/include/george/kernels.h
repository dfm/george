...
class MyLocalGaussianKernel : public Kernel {
public:
    ...
    double get_parameter (size_t i) const {
        if (i == 0) return param_x0_;
        if (i == 1) return param_log_w_;
        return 0.0;
    };
    void set_parameter (size_t i, double value) {
        if (i == 0) {
    ...
    void update_reparams () {
        reparam_inv_2w_ = get_reparam_inv_2w (
            param_x0_,param_log_w_
        );
        
    };

    double get_reparam_inv_2w (
        double x0,double log_w
    ) {
        return 0.5 * exp(-log_w);
    }
